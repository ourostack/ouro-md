...
- 2026-06-29 22:40 CI repair: shell `main` advanced again to package-relevant revision `26603ab5a9c4afa9388975bc3d9f8b5928bc769e`; refreshed the shell pins in Ouro MD and Workbench, then reran `scripts/check-shell-dependency.sh` and `scripts/check-app-control-deck.py` successfully in both consumer branches.
- 2026-06-29 22:48 CI repair: Ouro MD release freshness policy requires app/release-affecting dependency and preflight changes to carry a newer version. Bumped Ouro MD to `0.9.62`, updated release highlights, and reran `scripts/verify-release-version.sh`, `scripts/release-policy.sh freshness --mode pr --base-ref origin/main`, `scripts/check-shell-dependency.sh`, `scripts/check-app-control-deck.py`, and `git diff --check` successfully.
- 2026-06-29 23:00 CI repair: shell `main` advanced again to package-relevant revision `014cf09b21721987027dad19c7eaa50fb5d70566`; refreshed Ouro MD `Package.resolved` and reran `scripts/check-shell-dependency.sh`, `scripts/check-app-control-deck.py`, `scripts/verify-release-version.sh`, and `scripts/release-policy.sh freshness --mode pr --base-ref origin/main` successfully.
