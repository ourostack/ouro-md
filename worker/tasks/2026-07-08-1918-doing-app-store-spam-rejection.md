...
- 2026-07-09 Unit 7a complete: captured live exact-state discovery showing target version `0.9.80` absent, live app info id `9cfeeca8-42d0-4ad6-a2cb-fae6a95681f8`, existing `en-US` app info localization `b5bae77f-a94f-4b39-8ebd-841a6306b1c6`, uploaded build `827fa5b9-6994-41eb-bc75-ab3ca469a96f`, and a 29-request dry-run plan with four screenshots, no blockers, in-place app-info-localization patching, and final submit included pending Unit 7b gates.
- 2026-07-09 Unit 7b complete: voice reviewer converged; API reviewer found and re-reviewed fixes for existing app-info-localization reuse, review-detail ID capture, staged live apply preconditions, stale/unowned state rejection, and screenshot byte re-hashing before upload; request/apply suites, full Swift suite, final-submission preflight, and leak scan are green.
- 2026-07-09 Unit 7c complete: App Store Connect refused creating a second `0.9.80` version while the app was rejected, then accepted retargeting the existing rejected version object `7309944f-cbe8-4518-960c-444e6116ab46` to `0.9.80`; added and tested a narrow preflight-owned stale-ID allowance, required Unit 7c state for all later live stages, captured fresh preflight proof that the reused localization/review-detail/screenshot-set IDs belong to target `0.9.80`, created review submission `e87d8ecd-9682-4d79-9e60-14c23befd11e` in `READY_FOR_REVIEW`, recorded old-thread UI reply handling and final-submit guard evidence, sanitized private contact/raw asset artifacts, and reran full Swift tests green.
...
- 2026-07-09 Unit 8c pre-merge sanity Round 1 findings: stripped trailing whitespace from the committed CI failure log and replaced blanket coverage wording with the actual coverage policy/evidence split for automation contracts versus UI/app-surface validation.
