...
**What**: Extend the existing `resolveEditorLinkURL()` path used by both capture-phase `mousedown` and `click`; do not add a click-only reference handler. Update `Sources/OuroMD/web/index.html` and `Sources/OuroMD/web/bridge.js` so resolved `data-type="link-ref"` nodes keep reference syntax markers hidden in IR through CSS-only width/overflow/display rules that explicitly remain hidden under `.vditor-ir__node--expand`, while marker nodes remain in the DOM for serialization/editing. Call `vditor.vditor.lute.RenderJSON(currentMarkdown())`, walk `NodeLink` entries with `LinkType == 3`, collect each child `NodeLinkDest` and recursively flattened `NodeLinkText` in document order, and join the clicked DOM node by its ordinal among the active editor root's `span[data-type="link-ref"]` nodes. Normalize whitespace only for an integrity check between AST text and DOM label; on mismatch, fail closed. Suppress Vditor's IR `genLinkRefPopover` gesture so a plain external-reference click edits/positions the caret without opening or exposing the raw identifier; Source Code remains the identifier-editing surface. Cache only the exact `{markdown, mode, orderedLinks}` triple and invalidate it on input, `setValue`, `reloadValue`, and `rebuild`. If Unit 0 found clean-buffer round-trip red, make `getValue()` return exact `state.value` while `dirty == false`; otherwise leave that path unchanged.
**Output**: Render-only reference-marker handling and parser-owned destination routing.
**Acceptance**: Unit 1a is green; external reference links still open from the existing Command-mousedown path; plain external-reference clicks do not open or show Vditor's raw-reference popover; unresolved references remain visibly editable and non-clickable; every ordinal match validates normalized link text before opening; a deliberate AST/DOM order divergence is a no-op; label editing preserves reference syntax and destination; `window.ouro.getValue()` remains byte-identical before/after clicks and focus changes.

### ⬜ Unit 1c: Reference-style links — Coverage and refactor
**What**: Add edge coverage for repeated labels, whitespace/case normalization, escaped labels, optional titles, missing definitions, malformed destinations, definition blocks far from their uses, and a synthetic AST/DOM order mismatch that must fail closed. Cache parser-derived reference targets only against the exact current Markdown value and mode, and invalidate on every edit/reload/mode rebuild.
...
**What**: Define `reference_fixture="Tests/Fixtures/reference-links-and-anchors.md"` and `reference_roundtrip_out="$tmp/reference-links-and-anchors-roundtrip.md"` in `scripts/run-native-scenarios.sh`; add the three exact policy-declared lines for `--linktest`, `--roundtrip`, and `cmp`; do not duplicate fixture content in shell.
