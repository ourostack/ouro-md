...
**What**: Land rich `Tests/Fixtures/reference-links-and-anchors.md` for full/collapsed/shortcut/unresolved/navigation/heading/footnote behavior and strict `Tests/Fixtures/reference-links-roundtrip.md` for canonical full/shortcut references with bare destinations and no Lute-normalized constructs. Add `argValue("--linktest-file")` and `argValue("--linktest-artifact-dir")` seams in `Sources/OuroMD/main.swift`; update `--linktest` policy privacy for caller input/artifacts; and refactor `LinkTest.swift` into a rebuild-safe phase queue that loads the rich fixture, preserves the legacy scenario, and captures diagnostics. Export `OURO_LINKTEST_SCENARIO_TIMEOUT_SECONDS`, align inner/outer deadlines, reproduce both defects, capture raw Vditor values, and record a normalization matrix for collapsed references, titled/angle-bracket definitions, and footnote-to-reference-definition spacing as pre-existing vendored behavior.
...
**What**: Extend the Unit 0 phase queue with expected-red full/collapsed/shortcut presentation/routing assertions and expected-green standalone-render characterization. Add `argValue("--roundtrip-strict")` to `main.swift`; in strict mode make `RoundTrip.swift` fail before `MarkdownTidy.roundTripProbeOutput` unless raw `window.__ouroEditor.getValue()` and `window.ouro.getValue()` both equal the strict fixture input; keep ordinary `--roundtrip` behavior unchanged for known table/front-matter normalization. Extract and unit-test the strict byte helper with normalization-maskable mismatches. Add keyboard-caret, mouse-focus, and label-edit fidelity phases against the rich fixture.
...
**What**: After temp initialization/trap, define `link_fixture="Tests/Fixtures/reference-links-and-anchors.md"`, `roundtrip_fixture="Tests/Fixtures/reference-links-roundtrip.md"`, and `reference_roundtrip_out="$tmp/reference-links-roundtrip.md"`; add the three exact policy lines for rich link testing and strict raw round-trip/cmp; keep legacy link testing on the exported timeout budget; do not duplicate fixture content in shell.
