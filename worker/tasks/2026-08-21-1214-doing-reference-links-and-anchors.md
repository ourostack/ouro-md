...
**What**: Run `scripts/bump-version.sh 0.9.85`, then add a parser-safe fourth highlight while preserving positioning/`whatsNew` tokens. Update the four live-`--selftest`-manifest expectations in `OuroMDAppStoreRequestPlanTests`; in `OuroMDAppStoreApplyPlanTests`, update the live-plan `summary["targetVersion"]` expectation plus `livePreflight()`'s `targetVersion` and `uploadedBuild.version`, because that helper is validated against a live-manifest-derived plan. Leave only genuinely self-contained synthetic manifests pinned to `0.9.84`: request-plan fixtures with explicit `--manifest` and apply-plan `makeBlockedPlan`; then run all release-related tests/checks.
...
- 2026-08-24 21:33 CI repair: split large `LinkTest.handleProbe` tuple expressions after the clean GitHub runner compiler timed out; a clean release build, both live link modes, and the complete preflight passed again at PR head `896bb04`.
- 2026-08-24 21:55 Pre-merge sanity fixes: restored ordinary-link hover-only decoration, documented Lute/clipboard limitations, made cancellation and folder-budget fixtures deterministic, distinguished missing waiver baselines, and passed the complete preflight again.
