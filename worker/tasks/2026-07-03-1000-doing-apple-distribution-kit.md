...
- [x] Live Apple gates have named pass/blocker artifacts for App Store API auth, provider resolution, app-record discovery, certificate presence/import, profile creation/download, package validation, upload id, processed-build discovery, build association, and review-submission readiness.
- [x] Workbench and Spoonjoy have manifest/adoption PRs or generated adoption fixtures proving the kit is app-neutral and not overfit to Ouro MD; Spoonjoy coverage is dry-run/schema/manifest only unless the lane is explicitly promoted.
- [x] The `sign-apple-apps` skill and relevant app docs tell future agents to use the shared kit and explain the human/account boundary plainly.
...
**Output**: Workbench/Spoonjoy/skills check additions and red-run logs `unit-8a-workbench-red.log`, `unit-8a-spoonjoy-red.log`, and `unit-8a-skills-red.log`.
...
**Output**: Branches `worker/apple-distribution-kit-adoption` in Workbench, Spoonjoy, and skills; manifests/docs; and green-run logs `unit-8b-workbench-green.log`, `unit-8b-spoonjoy-green.log`, `unit-8b-skills-green.log`, and `unit-8b-ouro-md-green.log`.
...
- 2026-07-03 15:08 Unit 10 complete with named Apple blockers: App Store Connect auth/provider passed, `bot.ouro.md` bundle ID was created, Mac App Distribution and Installer certificates were created from local CSRs, Mac App Store profile was created/downloaded, and a signed App Store `.pkg` was built. `altool --validate-app` is blocked because the first App Store Connect app record for `bot.ouro.md` must be created in the UI; upload/build association/review are blocked behind that plus screenshot/app-preview assets.
