...
- Sources/OuroMD/MarkdownRenderer.swift:280 - Private visitor image byte cap storage; image inlining, supported MIME branches, remote URLs, empty URLs, and unsupported extensions are covered by `MarkdownRendererTests`.
- Sources/OuroMD/MarkdownRenderer.swift:282-283 - `MarkupVisitor.defaultVisit` fallback for future swift-markdown node types; current supported block/inline nodes touched by this follow-up are covered directly by `MarkdownRendererTests`.
