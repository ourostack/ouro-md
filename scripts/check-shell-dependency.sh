#!/usr/bin/env bash
#
# Keep Ouro MD dogfooding the current shared native app shell. SwiftPM branch
# dependencies still pin a concrete revision in Package.resolved, so a plain
# checkout can silently keep using an older shell main unless we guard it.
set -euo pipefail
...
resolved, identity, expected_location = sys.argv[1:]
...
for pin in data.get("pins", []):
    if pin.get("identity") == identity:
        ...
        if location != expected_location:
            raise SystemExit(f"{identity} pin location {location!r} != {expected_location!r}")
        state = pin.get("state", {})
