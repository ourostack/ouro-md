...
swift_version="$(ouro_md_source_version)"
...
# OuroMDRelease.swift (via the shared ouro_md_source_version helper), never
# hardcode it, or the packaged Info.plist could drift.
...
grep -q 'ouro_md_source_version' make-app.sh || fail "make-app.sh must derive VERSION via ouro_md_source_version"
