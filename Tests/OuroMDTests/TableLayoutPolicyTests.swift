...
final class TableLayoutPolicyTests: XCTestCase {
    func testStandaloneThemeLetsTablesUseViewportWithTableLocalScroll() {
        ...
        XCTAssertTrue(css.contains("width:max-content;max-width:var(--ouro-table-viewport);"))
        ...
        // No bleed margins on the base rule — narrow tables stay aligned with the
        // text column. (Standalone export has no JS to toggle the scrollable class,
        // so it carries no bleed at all.)
        XCTAssertTrue(css.contains(";margin:0.8em 0;table-layout:auto;"))
    ...
    func testEditorThemeUsesSameTablePolicyAsStandaloneRender() {
        ...
        XCTAssertTrue(css.contains("width:max-content!important;max-width:var(--ouro-table-viewport)!important;"))
        // Base table rule has no unconditional border-right AND no bleed margins;
        // both the scroll affordance and the negative bleed margins are applied
        // only to tables that actually overflow (bridge.js toggles the
        // ouro-table-scrollable class). Fitting tables get no stray grey strip and
        // stay aligned with the text column instead of being shoved into the margin.
        XCTAssertTrue(css.contains("-webkit-overflow-scrolling:touch;box-sizing:border-box!important;}"))
        XCTAssertTrue(css.contains(".vditor-reset table.ouro-table-scrollable{margin-left:min(0px, calc((100% - var(--ouro-table-viewport)) / 2))!important;margin-right:min(0px, calc((100% - var(--ouro-table-viewport)) / 2))!important;border-right:8px solid rgba(0,0,0,.08)!important;}"))
        XCTAssertFalse(css.contains("box-sizing:border-box!important;border-right:8px solid rgba(0,0,0,.08)!important;"))
        XCTAssertFalse(css.contains("margin:0.8em 0!important;margin-left:min(0px"))
