...
- 2026-07-09 17:00 -0700 Final reviewer blocker fixed: nested tracked repo files now run root-relative git pathspecs from the repository root. Red/green real-git evidence captured in `unit-5-nested-real-git-red.log` and `unit-5-nested-real-git-green.log`; full `swift test`, coverage, shell-boundary, PR preflight, and package-release passed again with package evidence in `unit-5-package-release-after-nested-fix.log`.
